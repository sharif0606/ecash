				...
				<div class="card">
					<div class="card-header">
						...
						<div class="d-flex justify-content-end">
							<a href="{{route(currentUser().'.addNewCouponForm')}}" class="btn btn-primary font-weight-bolder  waves-effect waves-float waves-light">
								<i class="la la-list"></i>New Coupon</a>
						</div>
					</div>
					<div class="card-body">
							    ...
							    <table id="myTable" class="table table-striped text-center table-bordered dt-responsive dataTable">
								    <thead class="thead-light">
                        				<tr>
                        					<th>Coupon Name</th>
                        					<th>Code</th>
                        					<th>Discount /<br> Num of Invoice</th>
                        					<th>Total Coupon</th>
                        					<th>Used</th>
                        					<th>Start At</th>
                        					<th>End At</th>
                        			...
                        			@if(count($allCoupon))
                        				@foreach($allCoupon as $coupon)
                        					<tr>
                        						<td>{{$coupon->name}}</td>
                        						<td>{{$coupon->code}}</td>
                        						<td>{{$coupon->discount?$coupon->discount."%":$coupon->noofbill." Invoice"}}</td>
                        						<td>{{$coupon->numberOfCoupon}}</td>
                        						<td>{{$coupon->stock}}</td>
                        						<td>
                        							@if($coupon->startAt)
                        								{{date('d-m-Y',strtotime($coupon->startAt))}}
                        							@endif
                        						</td>
                        						<td>
                        							@if($coupon->endAt)
                        								{{date('d-m-Y',strtotime($coupon->endAt))}}
                        							@endif
                        						</td>
                        						...
                        						<td>
                        							<div class="dropdown">
                        								...
                        								<a class="dropdown-item" href="{{route(currentUser().'.editCoupon',[encryptor('encrypt', $coupon->id)])}}">
                        									<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-2 me-50"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
                        									<span>Edit</span>
                        								</a>
                        								<a class="dropdown-item" href="{{route(currentUser().'.deleteCoupon', [encryptor('encrypt', $coupon->id)])}}">
