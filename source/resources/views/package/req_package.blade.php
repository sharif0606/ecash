...
@section('title', 'Request Package List')
@section('content')
                                        ...
                                        @if(count($allPackages))
                                            @foreach($allPackages as $package)
                                                <tr>
													...
													<td>
                                                        @if($package->status == 0)
                                                            <span class="btn btn-danger" title="Pending">Pending</span>
                                                        @else
                                                            <span class="btn btn-success" title="On Hold">Active</span>
                                                        @endif
                                                    </td>
                                                    <td>
														 @if($package->status==0)
														 ...
														 @else
														    <a href="{{route('superadmin.packageStatus', ['0',encryptor('encrypt', $package->id)])}}" onclick="return confirm('Are you sure to make this pending?')" class="btn btn-warning">Pending</a>
														 @endif
