...
<div class="content-wrapper container-xxl p-0">
    <div class="content-header row">
      <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
          <div class="col-12">
            <h2 class="content-header-title float-start mb-0">Service Edit</h2>
	...
	<div class="card">
	<!--begin::Form-->
	 <form class="form" action="{{ route(currentUser().'.updateService') }}" method="POST" enctype="multipart/form-data">
		@csrf
		<input type="hidden" value="{{encryptor('encrypt', $allserviceItems->id)}}" name="id">
		<div class="card-body">
            <div class="row form-group row-bill-to">
				<div class="col-md-12 mb-1">
					<label> Product Details: <span class="text-danger sup">*</span></label>
					<textarea class="form-control" id="exampleFormControlTextarea1" rows="5" placeholder="Product Details" name="product_detl" required>{{$allserviceItems->product_detl}}</textarea>
				</div>
				<div class="col-md-4 mb-1">
					<label> IMEI: <span class="text-danger sup">*</span></label>
					<input type="text" name="imei" class="form-control" value="{{$allserviceItems->imei}}"/>
				</div>
				<div class="col-md-4 mb-1">
					<label> Received Date: <span class="text-danger sup">*</span></label>
					<input type="text" name="receive_date" class="form-control invoice-edit-input date-picker" value="{{$allserviceItems->receive_date}}"/>
				</div>
						...
						<select class="invoiceto form-select" id="addcustomer_id" name="customer_id" required>
							...
							@if($allCustomer)
								@foreach ($allCustomer as $cust)
									<option value="{{$cust['id']}}" @if($allserviceItems->customer_id==$cust['id']) selected @endif>{{$cust['custCode']}} - {{$cust['name']}}</option>
								@endforeach
							@endif
						</select>
					</div>
...
<script>
	// init date picker
	var newdate = '{!! $allserviceItems->receive_date !!}';
	date = new Date();
