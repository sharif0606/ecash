...
<div class="content-wrapper container-xxl p-0">
	...
	<div class="card">
		...
		<!--begin::Form-->
		<form class="form" action="{{ route(currentUser().'.updateCustomer') }}" method="POST">
			@csrf
			...
			<div class="card-body">
				<div class="form-group row">
					<div class="col-lg-8 mb-1">
						...
						<input type="text" name="name" value="{{ $data->name }}" class="form-control @if($errors->has('name')) {{ 'is-invalid' }} @endif" placeholder="Customer Name" />
						@if($errors->has('name'))
					...
					<div class="col-lg-4 mb-1">
						...
						<input type="text" name="custCode" value="{{ $data->custCode }}" class="form-control @if($errors->has('custCode')) {{ 'is-invalid' }} @endif" placeholder="Customer Mobile No" />
						...
						<select name="contact_person" class="form-control">
							@if($allCustomer)
								@foreach($allCustomer as $c)
									<option value="{{ $c->id }}" @if($data->contact_person == $c->id) selected @endif >{{ $c->name.'-'. $c->custCode}}</option>
								@endforeach
							@endif
						</select>
					</div>
				...
				<div class="form-group row">
					<div class="col-lg-4 mb-1">
						...
						<select name="state_id"
							class="form-control" onchange="$('.zonop').hide(); $('.zonop'+this.value).show()" style="width: 100%; height:36px;">
							@if(count($allState) > 0)
					...
					<div class="col-lg-4 mb-1">
						<label>City:</label>
						<select name="zone_id" class="form-control" style="width: 100%; height:36px;">
							@if(count($allZone) > 0)
								@foreach($allZone as $zone)
									<option value="{{ $zone->id }}" class="zonop zonop{{$zone->stateId}}" @if($data->zone_id == $zone->id) selected @endif >{{ $zone->code.'-'. $zone->name}}</option>
								@endforeach
							@endif
						</select>
					</div>
